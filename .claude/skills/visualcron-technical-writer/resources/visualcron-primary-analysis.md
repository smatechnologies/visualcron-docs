...
>
> **Used by:** a documentation quality assessor — Template Adherence Review and Completeness subscore.
...
| 7 | Administration | Enable/disable, permissions, maintenance |
...
**Evaluation pass criteria:** At least 2 bullets present. Phrased as scenarios ("When you need to…", "If a file arrives…"), not capabilities ("This feature supports…").

---

### Section 3 — Why Would You Use It? (Value)

Benefits written in plain language. Focus on outcomes: easier, faster, more reliable, more automated, reduced manual effort. Avoid marketing language.
...
- Which permissions or permission groups can access or configure it
- Routine maintenance tasks (if any)

**Evaluation pass criteria:** All three sub-topics addressed. Permission and role names use VisualCron terminology.
...
- Authentication — what credentials, tokens, or accounts are involved
- Authorization — which permissions or groups can access or modify this feature
- Data security — what data is stored, transmitted, or exposed (note AES-256 encryption of stored values and TLS between Client and Server where relevant)
- Sensitive data — any credentials, secrets, or regulated data handled
...
- Monitoring — what signals indicate the feature is healthy (Grid state, logs, Server monitor, SLA)
- Alerts — what conditions should trigger a Notification
- Performance and scaling — known limits, load considerations, or tuning guidance

**Evaluation pass criteria:** All three sub-topics addressed. Specific signals, alert conditions, or thresholds stated where applicable.
...
The **Structure / Template Adherence subscore** (weighted 15%) evaluates whether sections appear in order, use the correct format (table, numbered list, and so on), and meet the structural requirements stated above — not just whether content is present.

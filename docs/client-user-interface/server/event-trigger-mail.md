...
The Email event trigger is able to monitor a mailbox through the POP3, IMAP4, or Microsoft Graph protocol. The actual connection properties are defined in the [Connections](../server/global-connections) window. Based on subject, sender and content different actions can be performed with the email.

:::tip Microsoft Graph support

When using a [Microsoft Graph](../server/connection-microsoft-graph) connection, the Mail Trigger polls the Graph API for new messages. All actions (mark as read, delete, move, copy, save email, save attachments, forward) are fully supported. Server-side OData filtering is used for From, Subject, and read state conditions to improve performance on large mailboxes.
...
Select an existing [POP3](../server/connection-pop3)/[IMAP4](../server/connection-imap4)/[Microsoft Graph](../server/connection-microsoft-graph) connection, if not existing you need to create one first. Click the Settings icon to open the Manage Connections dialog. Once a connection is added you can select it in the combo box.

**Mail folder**

When VisualCron connects it uses the default folder retrieved from the mail server. If you want to change the folder you monitor you select "Use specified folder" and write the folder name in the text box. When using Microsoft Graph, you can specify a folder by display name (e.g., `Inbox`, `Archive`). VisualCron resolves the folder name to the Graph folder ID automatically.
...
:::tip Microsoft Graph filtering

When using a Microsoft Graph connection, From address, Subject, and Read state conditions that use Equal, Not equal, Contains, or Don't contain operators are pushed to the server as OData filter queries. This significantly improves performance when monitoring large mailboxes, as only matching messages are downloaded. Body and Received date conditions are evaluated client-side after retrieval.
...
This controls what happens with the message on the mail server. It is recommended that you delete or move the mail or VisualCron will check that email again in the next polling interval. If you leave the message on the server you can choose to mark it as read (this option is available in IMAP4 and Microsoft Graph because it does not exist in POP3 protocol). Note that VisualCron does not care if it is read or not - VisualCron has it's internal list to check if email has been checked or not.

**Copy message to folder**

This option copies a mail to another folder. It does not delete it unless you set Delete message on Server in previous setting. This option is available when using IMAP4 or Microsoft Graph as protocol mode. Not available for POP3.

**Forward email**

Forward the matched email to one or more recipients. Specify the recipient email addresses separated by the configured separator. This option is available for all protocol modes including Microsoft Graph.
...
When using Microsoft Graph, the email is downloaded as a .eml file (MIME format) from the Graph API. Attachments are downloaded individually via the Graph attachments endpoint.

...
The SharePoint - Delete file(s) Task deletes files from the SharePoint library using a file filter to select which files to remove.
...
![](../../../../../static/img/Client%20User%20Interface/Main%20Menu/Server/Jobs/Job%20Tasks/Tasks/Sharepoint%20Tasks/delete%20files.png)

**Delete file(s) > Location** tab
...
A wildcard pattern used to match the files to delete (e.g. `*` for all files, `*.txt` for text files only).
...
When checked, the number of matched files processed is limited to the specified value.

**Delete file(s) > Test** tab

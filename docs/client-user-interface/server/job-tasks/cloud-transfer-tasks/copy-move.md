...
The Cloud - Copy/Move item(s) Task copies or moves files between folders within a single cloud connection (Amazon S3, Box, Dropbox, Google Drive, Google Storage, Microsoft Azure or OneDrive). Where the cloud service supports it, this is a server-side copy — files are not downloaded to the VisualCron server and re-uploaded.
...
Source and destination must be on the same Connection. There is no separate destination connection — the *Destination folder* is a path within the same bucket/container/drive selected on the *Remote file filter* sub tab. To move files between two different cloud accounts use the [Sync](sync) task or chain a [Download](download-file) and [Upload](upload-file) task together.
...
The folder within the same Connection where the matched items will be copied or moved to. Click the *Folder* icon to browse the cloud and pick the destination. Required.

**Keep modified/created date**

When checked, the modified and created dates from the source file are preserved on the destination file. Only fully supported on Google Drive; on other cloud services the destination timestamps are set by the service and this option has no effect.

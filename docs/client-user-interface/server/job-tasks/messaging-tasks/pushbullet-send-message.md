...
The Pushbullet - Send message Task sends a message to one or more Pushbullet devices, contacts and/or channels.
...
**Pushbullet - Send message > Recipients** sub tab
...
The Recipients section has three nested sub tabs — *Devices*, *Contacts* and *Channels* — that let you choose where the message is delivered. At least one recipient must be selected across the three tabs.

Each recipient sub tab works the same way:

* _Send to all_ - when checked, the message is sent to every recipient of that type and the grid is disabled

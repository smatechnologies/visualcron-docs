...
This task supports **SMTP**, **Exchange EWS**, and **Microsoft Graph** connections. When using a Microsoft Graph connection, emails are sent via the Graph API. S/MIME and PGP signing/encryption are supported across all protocols. Large attachments (over 3 MB) are automatically handled via the Graph upload session flow when using Microsoft Graph.
...
You have to select a Connection. All connections are "global" and you can add new connections by clicking on Manage connection. Supported connection types: [SMTP](../../../server/connection-smtp), [Exchange EWS](../../../server/connection-exchange-ews), and [Microsoft Graph](../../../server/connection-microsoft-graph).
...
You can choose to Sign and/or Encrypt the message in S/MIME or PGP/MIME format. Select appropriate method. When selected new tabs Sign/Encrypt will be enabled which needs further settings to complete the Task. When using a Microsoft Graph connection, signed and encrypted messages are sent via the raw MIME upload endpoint to preserve cryptographic formatting.
...
When using a Microsoft Graph connection, attachments larger than 3 MB are uploaded using the Graph upload session flow. This handles large files automatically without requiring additional configuration.
